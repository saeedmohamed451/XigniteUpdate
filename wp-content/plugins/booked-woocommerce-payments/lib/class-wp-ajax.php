...
class Booked_WC_Ajax {
	...
	public function add_appointment_to_cart() {
		...
		if ( !$app_id ) {
			$response->add_message(__('Appointment ID is not defined.', 'booked-woocommerce-payments'));
			$response->create();
		}
		...
		$response->add_message(__('Appointment has been added to the cart', 'booked-woocommerce-payments'));
