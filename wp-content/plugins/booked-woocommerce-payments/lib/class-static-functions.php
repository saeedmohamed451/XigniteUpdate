...
class Booked_WC_Functions {
	...
	public static function booked_new_appointment_created( $appointment_id = null ) {
		...
		$cart_page_id = Booked_WC_Helper::get_checkout_page();
		$cart_page_url = get_permalink($cart_page_id);

		// send the visitor to the checkout page
		wp_redirect($cart_page_url); exit;
	...
	public static function booked_fea_shortcode_appointments_buttons($button_html, $appt_id) {
		...
		if ($appointment->order_id):
			...
			$button_html = '<div class="booked-fea-buttons">';
				$button_html .= ($status_class == 'pending' ? '<button data-appt-id="'.$appt_id.'" class="approve button button-primary">'.__('Approve','booked-frontend-agents').'</button>' : '');
			$button_html .=	'</div>';
			return $button_html;
		else:
			return $button_html;
	...
	public static function remove_default_confirmation_emails() {
		...
		if (isset($_POST)):
			...
			if ( $booked_wc_product && $booked_disable_confirmation_emails ) {
				...
				remove_action( 'booked_reminder_email', 'booked_mailer', 10 );
				remove_action( 'booked_admin_reminder_email', 'booked_mailer', 10 );				
			} else {
				return;
			}
